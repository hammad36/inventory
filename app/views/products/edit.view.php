<section class="bg-gradient-to-br from-blue-200 via-white to-blue-100 min-h-screen flex items-center justify-center relative">
    ...
    <div class="absolute top-4 left-4 sm:top-6 sm:left-6 z-30">
        <a href="/products?category_id=<?= htmlspecialchars($product->getCategoryId()) ?>"
    ...
    <div class="relative z-20 mx-auto w-full max-w-lg bg-white rounded-lg shadow-xl my-16 px-8 py-12 transform transition hover:scale-105 duration-300">
        ...
        <form action="/products/edit?product_id=<?= htmlspecialchars($product->getProductId()) ?>" method="POST" class="space-y-6 mt-6">
            <!-- Product Name -->
            <div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" required
                    class="w-full px-4 py-2 mt-1 text-gray-900 bg-gray-50 border rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 transition duration-300"><?= htmlspecialchars($product->getDescription()) ?></textarea>
            </div>

            <!-- Quantity -->
            <div>
                <label for="quantity" class="block text-sm font-medium text-gray-700">Quantity</label>
                <input type="number" name="quantity" id="quantity" value="<?= htmlspecialchars($product->getQuantity()) ?>" min="0" required
                    class="w-full px-4 py-2 mt-1 text-gray-900 bg-gray-50 border rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 transition duration-300">
            </div>
            ...
            <div>
                ...
                <input type="number" name="unit_price" id="unit_price" value="<?= htmlspecialchars($product->getUnitPrice()) ?>" step="0.01" required
                    class="w-full px-4 py-2 mt-1 text-gray-900 bg-gray-50 border rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 transition duration-300">
            </div>
            ...
            <div>
                ...
                <select name="category_id" id="category_id" required
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= htmlspecialchars($category->getCategoryId()) ?>" <?= $category->getCategoryId() == $product->getCategoryId() ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <input type="url" name="photo_url1" id="photo_url1" value="<?= htmlspecialchars($photoUrls[0] ?? '') ?>"
                    class="w-full px-4 py-2 mt-1 text-gray-900 bg-gray-50 border rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 transition duration-300">
            </div>
            <div>
                <label for="photo_url2" class="block text-sm font-medium text-gray-700">Photo URL 2</label>
                <input type="url" name="photo_url2" id="photo_url2" value="<?= htmlspecialchars($photoUrls[1] ?? '') ?>"
                    class="w-full px-4 py-2 mt-1 text-gray-900 bg-gray-50 border rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 border-gray-300 transition duration-300">
            </div>
